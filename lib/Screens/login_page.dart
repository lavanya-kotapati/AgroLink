  ...
  Widget build(BuildContext context) {
    return BlocConsumer<LoginBloc, LoginState>(
      listener: (context, state) async {
        if (state is LoginEventLoadingState) {
          AppLoader.show(context, message: "Please wait Logging in");
        }
        if (state is LoginEventLoadedState) {
        ...
        if (state is LoginEventErrorState) {
          ...
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(SnackBar(content: Text(state.errorString)));
